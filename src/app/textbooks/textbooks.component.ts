...
export class TextbooksComponent implements OnInit {
  ...
  constructor(private actRout: ActivatedRoute, public fetch: DataFetcherService, private sender: SenderService) { }
  ...
  ngOnInit() {
    ...
    this.fetch.textbookData().subscribe(data=>{
      this.faker = data;
      for (let i = 0; i < this.faker.length; i++){
          if(this.faker[i].department==this.query  && this.faker[i].referal=='textbook') {

...
@Component({
  selector: 'app-soft-readers-shw',
  templateUrl: './soft-readers-shw.component.html',
  styleUrls: ['./soft-readers-shw.component.css']
})
export class SoftReadersShwComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.fetch.softData().subscribe(data=>{
      this.faker = data;
      for (let i = 0; i < this.faker.length; i++){
          if(this.faker[i].department==this.query  && this.faker[i].referal=='softreader') {

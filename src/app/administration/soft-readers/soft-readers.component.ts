...
@Component({
  selector: 'app-soft-readers',
  templateUrl: './soft-readers.component.html',
  styleUrls: ['./soft-readers.component.css']
})
export class SoftReadersComponent implements OnInit {
  ...
  submit(){
    ...
    this.server.submitCover(this.coverImg).subscribe(dataCover=>{
      ...
      this.server.submitImgAuthor(this.imgAuthor).subscribe(dataImage=>{
        this.server.submitSoftReader(this.book, this.cover, this.authorImage, this.fileArr).subscribe(data=>{
        console.log(data);
        //if(data==true){
          this.id = data;
